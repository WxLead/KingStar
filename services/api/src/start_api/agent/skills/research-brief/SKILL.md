...
1. 用 web_search / web_fetch 收集公开资料；可用 `library_search` 查看本地是否已有相关文献（只读）。
2. 边写边发布：用 `mcp__start__publish_report` 流式输出终稿。
3. 聊天里只给短摘要；完整正文放在报告产物中。
4. **收尾只反问一个问题**（例如是否要把某篇候选论文入库，或是否需要版面解析）。等用户下一条消息再执行，不要连环追问。

## 默认不要做（除非用户本条消息明确要求）

- 不要调用 `upload_from_url`（入库）
- 不要调用 `parse_document`（版面解析）
- 不要调用 `translate_document`（翻译）

「调研 / 综述 / 写报告」本身 **不等于** 入库或解析。可在报告与摘要中列出候选论文链接/标题，供收尾那一个问题引用。

仅当用户明确说「入库 / 导入 / 解析 / 翻译」或点名某 URL 要导入时，才调用对应工具。
